...
| 2026-08-19 | Craig | 0.8 | **A flow carries several labels, and the schema breaks for it.** `labels[]` replaces the top-level `verdict`, so `verdict` becomes one entry among several and a second kind of assertion costs no new field. **`threat-name`** joins it, publishing what the inline device called the threat; it is tier-1 only for now and is **omitted** rather than nulled on a Suricata-only flow, which makes extending it additive. One per flow, chosen by tier and then earliest detection (`sid` breaking exact ties, because "earliest" is not a total order and the label has to be stable across runs). Tier 2's threat name stays in `sources[].threat` and is not promoted — a deliberate trade for one trainable value per flow. `schema_version` moves to **2.0**: the first genuine break, since a 1.0 consumer finds no `verdict` key at all. Goal 6 stands as verified at Phase 2 and was never a promise about later changes (#138). |
| 2026-08-18 | Craig | 0.7 | **The CLI contract reopens, and the default changes.** v0.4 closed it with "Phase 2 adds no flags", which held until the built default turned out to be the wrong default: `flabel <capture>` ran Tier 1 + Tier 2, so evaluating the NGFW alone meant paying a full 85,000-rule Suricata load on every replay and reading an output that mixed the tier under test with the open-source baseline. The bare command is now **Tier 1 only**, `--both` is the old default renamed, and `--offline` is unchanged and still permanent. Zeek runs in all three modes — it is the flow substrate, not a tier. `run.mode` widens to `replay|offline|both` and `tiers_unavailable` narrows to attempted-and-lost, which also fixes a Phase 1 constant that had been publishing `mode: offline` on runs that replayed past the firewall (#132). |
...
**Goal 6 was met and is not open-ended.** It was verified at Phase 2 (#128): tier 1 arrived as new
`sources[]` entries, the schema version did not move, and a Phase 1 consumer parsed the output
unchanged. It says nothing about later changes, and #138 is one — `labels[]` replaced the top-level
`verdict` and `schema_version` is now `"2.0"`. Goal 6 is not retroactively unmet; it is simply not a
promise that the schema never breaks again.
