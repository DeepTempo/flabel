...
| 5 | **`tools/flabel-run` only, never `flabel` itself.** The labelling run succeeded, its result **was published**, and the store was **not updated** — `flabel-ingest` failed. Same reasoning as 4 and a code of its own for the same reason: the labels are intact both on the box and in the bucket, so reusing 1 would tell a batch caller to discard a capture that succeeded, and reusing 4 would say the tarball is missing when it is not. The recovery is to re-run the ingest, not the capture — and re-indexing is free, because `flabel-ingest` short-circuits on the `runs` commit marker (spec-label-store §7.4). Added by LS-5; the ordering is always archive-then-index (§7.5 there), so 5 is strictly later than 4. |
